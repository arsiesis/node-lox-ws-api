...
var API = function (host, username, password, reconnect, security, remote, protocol) {
    this._host = host;
...
API.prototype.connect = function() {
    var connection = new Connection(this._host, this._protocol === 'https' ? 'wss' : 'ws');
...
API.prototype.perform_version_check = function(connection) {
    if(this._remote){
        this._auth_class = require('./Auth/Tunnel.js');
        connection.connect();
    }else{
        var that = this;
        
        axios.get(`${this._protocol}://${this._host}/jdev/cfg/api`, {httpsAgent: this._protocol === 'https' ? new https.Agent({rejectUnauthorized: false}) : null})
            .then((response) => {
