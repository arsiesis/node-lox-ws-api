...
TokenEnc.prototype._get_public_key = function() {
    var that = this;
    axios.get(`${this._api._protocol}://${this._host}/jdev/sys/getPublicKey`, {httpsAgent: this._api._protocol === 'https' ? new https.Agent({rejectUnauthorized: false}) : null})
        .then((response) => {
            that._parse_public_key(response.data);
...
TokenEnc.prototype._parse_public_key = function(data) {
...
TokenEnc.prototype._generate_session_key = function() {
    try{
        ...
        console.error(err);
        this._session_key = "InvalidKeySetBecauseBadPublickKey";
    }
...
TokenEnc.prototype._register_getkey2_response = function() {
    ...
    this._api.command_chain.push({
        ...
        'callback': function(loxone_message) {
            if (loxone_message.value.hashAlg == 'SHA256') {
                that._sha = 'sha256';
            }
            var key = new Buffer(loxone_message.value.key, 'hex').toString('utf8');
            var salt = loxone_message.value.salt;
            var pw_hash = crypto.createHash(that._sha).update(that._password + ':' + salt).digest('hex').toUpperCase();
            var hmac = crypto.createHmac(that._sha, key);
            ...
            that._connection.send(that._enc_command('jdev/sys/gettoken/' + hash + '/' + that._username
                + '/2/00a5b9e8-4143-11eb-b3780242ac130002/nodeloxwsapi'));
        },
        'onetime': true,
    });
};
